...
def can_run_script() -> bool:
    print("[STEP] Checking last post status in ashwini_fb_topics.json...", flush=True)
    topics_file = Path("ashwini_fb_topics.json")
    if not topics_file.exists():
        print("[WARNING] 'ashwini_fb_topics.json' nahi mili. Proceeding by default...", flush=True)
        return True
...
def get_next_topic_from_json() -> str:
    print("[STEP] Reading ashwini_fb_topics.json...", flush=True)
    topics_file = Path("ashwini_fb_topics.json")
    if not topics_file.exists():
        raise FileNotFoundError("❌ 'ashwini_fb_topics.json' file nahi mila.")
    
    with topics_file.open("r", encoding="utf-8") as f:
        topics = json.load(f)
    
    if not topics:
        raise ValueError("❌ 'ashwini_fb_topics.json' khali hai.")
    ...
    raise ValueError("❌ 'ashwini_fb_topics.json' mein koi naya unprocessed topic nahi mila.")


def update_topic_status_in_json(topic_text: str):
    print("[STEP] Updating topic status in ashwini_fb_topics.json...", flush=True)
    topics_file = Path("ashwini_fb_topics.json")
    if not topics_file.exists():
        return
    ...
    print("[OK] Topic status successfully updated (content_generated=True) in ashwini_fb_topics.json", flush=True)
...
def run():
    ...
    # Last post check condition validation - status check FIRST before any JSON operations
    if not can_run_script():
        print("[INFO] Conditions match nahi hui. Exiting script gracefully...", flush=True)
        sys.exit(0)

    # Har baar new run par post.json ke content ko clear kar dena - JSON initialization AFTER status check passes
